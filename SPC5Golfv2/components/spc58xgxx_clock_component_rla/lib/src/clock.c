...
/**
 * @file    clock.c
 * @brief   SPC58xGxx clock subsystem source.
 *
 * @addtogroup CLOCK
 * @{
 */
...
static void SPCSetPFlashController(uint32_t clk_freq) {
  ...
  if ((clk_freq > 0UL) && (clk_freq <= 180000000UL)) {
    PFLASH_1.PFCR1.B.APC = 0;
    PFLASH_1.PFCR1.B.P0_BFEN = 1;
    PFLASH_1.PFCR2.B.P1_BFEN = 1;
    if (clk_freq <= 30000000UL) {
      /* 0 < clk_freq <= 30MHz*/
      PFLASH_1.PFCR1.B.RWSC = 0;
    } else if (clk_freq <= 60000000UL) {
      /* 30MHz < clk_freq <= 60MHz*/
      PFLASH_1.PFCR1.B.RWSC = 1;
    } else if (clk_freq <= 90000000UL) {
      /* 60MHz < clk_freq <= 90MHz*/
      PFLASH_1.PFCR1.B.RWSC = 2;
    } else if (clk_freq <= 120000000UL) {
      /* 90MHz < clk_freq <= 120MHz*/
      PFLASH_1.PFCR1.B.RWSC = 3;
    } else if (clk_freq <= 150000000UL) {
      /* 120MHz < clk_freq <= 150MHz*/
      PFLASH_1.PFCR1.B.RWSC = 4;
    } else {
      /* 150MHz < clk_freq <= 180MHz*/
      PFLASH_1.PFCR1.B.RWSC = 5;
    }
  }
}

/**
 * @brief   Setup PRAM controller
 *
 * @param[in] clk_freq  clock frequency for PRam Controller
 *
 * @notapi
 */
static void SPCSetPRamController(uint32_t clk_freq) {

  if ((clk_freq > 0UL) && (clk_freq <= 180000000UL)) {
    if ((clk_freq > 0UL) && (clk_freq <= 120000000UL)) {
      /* 0 < clk_freq <= 120MHz */
      PRAM_0.PRCR1.B.FT_DIS = 0;
      PRAM_1.PRCR1.B.FT_DIS = 0;
      PRAM_2.PRCR1.B.FT_DIS = 0;
      PRAM_3.PRCR1.B.FT_DIS = 0;
    } else {
      /* 120MHz < clk_freq <= 180MHz */
      PRAM_0.PRCR1.B.FT_DIS = 1;
      PRAM_1.PRCR1.B.FT_DIS = 1;
      PRAM_2.PRCR1.B.FT_DIS = 1;
      PRAM_3.PRCR1.B.FT_DIS = 1;
    }
  }
}
...
/**
 * @brief   SPC58xGxx clocks and PLL initialization.
 *
...
void spc_clock_init(void) {
  ...
  MC_CGM.AC2_DC0.R        = SPC5_CGM_AC2_DC0_BITS;
  MC_CGM.AC2_DC1.R        = SPC5_CGM_AC2_DC1_BITS;
  ...
  /* PRam Controller settings.*/
  SPCSetPRamController(SPC5_XBAR_CLK);
